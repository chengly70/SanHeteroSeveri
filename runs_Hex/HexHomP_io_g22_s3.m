% using 18 types of C<->P cell, with GRADual I.C.; ALL types well-reprsnt.
%(Peripheral at beginning of L.C., Center at the end)
% using nn_coupVryStrHex.m to VARY the strength of gap (stronger in P)
%FILE NAME: [oi/io]=Init.Cond; g[j]=.25,.5,2,4nS for g_param, 
%s[j]=1(ggap C->P 15X), 2(ggap P->C 15X), 3(constant ggap 0.5*g_param)
...
ind_j = length(isi_L)*ones(size(ind_j)); % all Peripheral cells

g_parm=0.00125; %gapJ strength, in microS
...
V_out=V_out(:,1:Nz); %only save voltages; longer run

save dhexhomP_io_g22_s3 g_parm gp_lim Nz X_l Y_l V_out time G_cup ind_j 